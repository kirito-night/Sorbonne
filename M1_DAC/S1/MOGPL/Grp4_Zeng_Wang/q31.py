...
#w = np.array([1, 1]) # pour w = (2,1)
w = np.array([9, 1])# pour w = (10,1)
#w = np.array([0, 1]) # pour retrouver le cas non equitable dans l'article
...
for i in range(len(b)):
    for j in range(len(b[i])):
        print("b_",i,j, ": ", b[i,j].x)
tmp = 0
for i in range(len(z)):
    tmp += z[i].x
    print("z is :",z[i].x)
print("satisfaction moyenne :",tmp/len(z))