...
obj = LinExpr();
obj = sum([M[i][j]*cout1[i][j] for i in range(len(arcs)) for j in range(len(arcs[i]))])
...
for indice in range(1,6):
    s=L[indice]
    m.addConstr(quicksum(M[indice][j] for j in range(len(M[indice]))) - quicksum(M[i][j] for i in range(len(arcs)) for j in range(len(arcs[i])) if arcs[i][j]==s)  == 0)


m.addConstr(quicksum(M[i][j]*cout1[i][j] for i in range(len(arcs)) for j in range(len(arcs[i]))) - z[0] == 0)
m.addConstr(quicksum(M[i][j]*cout2[i][j] for i in range(len(arcs)) for j in range(len(arcs[i]))) - z[1] == 0)
