...
def plot2DSet(desc, label):
   ...
   # TODO: A Compléter
    ...
    plt.scatter(negatifs[:, 0], negatifs[:, 1], marker='o',
                color="red")  # 'o' rouge pour la classe -1
    plt.scatter(positifs[:, 0], positifs[:, 1], marker='x',
                color="blue")  # 'x' bleu pour la classe +1
...
def genere_dataset_uniform(p, n, min, max):
    ...
    data1_label = np.asarray(
        [-1 for i in range(0, n)] + [+1 for i in range(0, n)])
...
def crossval(X, Y, n_iterations, iteration):
    left = len(Y) * iteration // n_iterations
    right = len(Y) * (iteration+1) // n_iterations
    Xapp = np.concatenate((X[:left], X[right:]))
    Yapp = np.concatenate((Y[:left], Y[right:]))
    Xtest = X[left:right]
    Ytest = Y[left:right]
    return Xapp, Yapp, Xtest, Ytest
...
def crossval_strat(X, Y, n_iterations, iteration):
    left_0 = len(Y[Y==-1]) * iteration // n_iterations
    right_0 = len(Y[Y==-1]) * (iteration+1) // n_iterations
    left_1 = len(Y[Y==1]) * iteration // n_iterations
    right_1 = len(Y[Y==1]) * (iteration+1) // n_iterations
    index_test = np.concatenate((np.where(Y==-1)[0][left_0:right_0], np.where(Y==1)[0][left_1:right_1]))
    index_test_bool = np.zeros(len(Y),dtype=bool)
    index_test_bool[index_test] = True
    Xapp = X[index_test_bool==False]
    Yapp = Y[index_test_bool==False]
    Xtest = X[index_test_bool]
    Ytest = Y[index_test_bool]
    return Xapp, Yapp, Xtest, Ytest
